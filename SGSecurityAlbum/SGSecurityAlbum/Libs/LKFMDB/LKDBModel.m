    ...
    [lkDB.dbQueue inDatabase:^(FMDatabase *db) {
        ...
        while ([resultSet next]) {
            ...
            for (int i=0; i< model.columeNames.count; i++) {
                ...
                {
                    [model setValue:[resultSet dataForColumn:columeName] forKey:model.propertyNames[i]];
                }
                else {
...
+ (NSDictionary *)getPropertys
{
    ...
    for (i = 0; i < outCount; i++) {
        ...
        /*
         ...
         在源程序中增加对源程序中nsdata的判断
         */
        if ([propertyType hasPrefix:@"T@"]) {
            if ([propertyType rangeOfString:@"NSData"].length) {
                [proTypes addObject:SQLBLOB];
            }else if([propertyType rangeOfString:@"NSString"].length)
            {
